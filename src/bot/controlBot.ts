...
async function apiPost(path: string, body: any) {
  const { data } = await axios.post(`${API}${path}`, body, {
    headers: { 'x-api-key': process.env.API_KEY || 'dev-key-123' },
  });
  return data;
}
...
bot.onText(/^\/add\s+([^\s]+)\s+(userbot)\s+([\w-]+)/i,
  async (msg: Message, m: RegExpExecArray | null) => {
    ...
    try {
      if (!m) { bot.sendMessage(chatId, 'Usage: /add <t.me/slug|@slug> userbot <profileId>'); return; }
...
bot.onText(/^\/list$/i, async (msg: Message) => {
  try {
    ...
    if (!Array.isArray(list) || list.length === 0) {
      bot.sendMessage(msg.chat.id, 'No channels configured.');
      return;
    }
    const lines = list.map((c: any) =>
      `${c.active ? '✅' : '⏸️'} ${c.slug} (${c.mode}) [profile:${c.buyProfileId || '-'}]`
    ).join('\n');
...
bot.onText(/^\/remove\s+([^\s]+)/i, async (msg: Message, m: RegExpExecArray | null) => {
  const chatId = msg.chat.id;
  try {
    if (!m) { bot.sendMessage(chatId, 'Usage: /remove <slug>'); return; }
    const slug = parseSlug(m[1]);
    await apiPost('/channels/toggleBySlug', { slug, active: false });
    bot.sendMessage(chatId, `🛑 Disabled ${slug}`);
  } catch (e: any) {
    bot.sendMessage(chatId, `❌ /remove failed: ${e.response?.data?.error || e.message}`);
  }
});
...
bot.onText(/^\/status\s+([\w-]+)/i, async (msg: Message, m: RegExpExecArray | null) => {
  const chatId = msg.chat.id;
  if (!m) { bot.sendMessage(chatId, 'Usage: /status <profileId>'); return; }
  const profileId = m[1];
  try {
...
Treasury: ${st.treasury || '(not set)'}`;
    bot.sendMessage(chatId, body, { reply_markup: kb });
  } catch (e: any) {
    bot.sendMessage(chatId, `❌ /status failed: ${e.response?.data?.error || e.message}`);
  }
});
...
console.log('Control bot ready. Use /add @slug userbot <PROFILE_ID> and /status <PROFILE_ID>.');
